#!/usr/bin/env python
##
## purpose: calculate the Manhatton distance between pair-wise sequence
## input is the netMPHpan xlsx for multiple sequence and multiple allel (from run_netMHCpan.sh)

## ## output is the distance matrisx for pair-wise seq
...
## assume to induce cross-immunity with same aa at TCRf binding site of core region
...
def main():
    ...
    ## input is the netMPHpan xlsx for multiple sequence 
    ## for single allel, load the xls file and filter by cutoff
    data = pd.read_csv(args.input,sep="\t",skiprows=0)
    data_epitope = data.loc[data['EL_Rank'] <2] ## use 2 as cutoff to filter epitope
