...
## This version is used to create jobs for each allel on gacrc
...
do
	...
	cat sub.sh > netMHCpan_sub.sh
	echo "netMHCpan -f $seq_pep -l $epitope_len -a $allele -xls -xlsfile $outxls" >> netMHCpan_sub.sh
	#sbatch netMHCpan_sub.sh
	echo "submit netMHCpan job for allel $allele"  	
